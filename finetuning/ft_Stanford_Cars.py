...
DATA_DIR = os.path.join(PARENT_DIR, "data/stanford_cars")
...
# =============================================================================
# 2. DATA HANDLING (STANFORD CARS .MAT PARSER)
# =============================================================================
class StanfordCarsDataset(Dataset):
    def __init__(self, root_dir, is_train=True, transform=None):
        ...
        # MODIFIED: Updated path to match car_devkit/devkit/ structure
        annos_path = os.path.join(root_dir, 'car_devkit', 'devkit', 'cars_train_annos.mat')
        mat_data = scipy.io.loadmat(annos_path)
        annotations = mat_data['annotations'][0]
        
        dataset_items = []
        for anno in annotations:
            img_name = str(anno[-1][0])
            class_idx = int(anno[-2][0][0]) - 1 
            
            # MODIFIED: Updated path to match nested cars_train/cars_train/ structure
            img_path = os.path.join(root_dir, 'cars_train', 'cars_train', img_name)
...
train_ds = StanfordCarsDataset(DATA_DIR, is_train=True, transform=transform_train)
val_ds = StanfordCarsDataset(DATA_DIR, is_train=False, transform=transform_val)
...
def train_phase(epochs, optimizer, scheduler, stage, phase_name):
    ...
    log_to_file(f"Epoch\tTrain_Loss\tVal_Acc\t\tLR")
    ...
    for epoch in range(epochs):
        ...
        log_to_file(f"{epoch+1:02d}/{epochs}\t{train_loss/len(train_loader):.4f}\t\t{acc*100:.2f}%\t\t{current_lr:.6f}\t{flag}")
