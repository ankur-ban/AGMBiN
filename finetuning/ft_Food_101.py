...
RAW_DATA_DIR = os.path.join(PARENT_DIR, "data/food_101/images")
SPLIT_DATA_DIR = os.path.join(PARENT_DIR, "data/food_101_split")
...
def train_phase(epochs, optimizer, scheduler, stage, phase_name):
    ...
    # Header updated for time
    log_to_file(f"Epoch\tTrain_Loss\tVal_Acc\t\tLR\t\tTime(m:s)\tStatus")

    for epoch in range(epochs):
        epoch_start_time = time.time() # Capture start time
        model.train()
        train_loss = 0.0
...
if __name__ == "__main__":
    total_start_time = time.time() # Capture start time for total execution
