...
DATA_DIR = os.path.join(PARENT_DIR, "data/oxford_flowers")
...
# =============================================================================
# 2. DATA HANDLING (OXFORD 102 FLOWERS FORMAT)
# =============================================================================
class OxfordFlowersDataset(Dataset):
    def __init__(self, root_dir, is_train=True, transform=None):
        ...
        splits_path = os.path.join(root_dir, 'setid.mat')
        
        if not os.path.exists(labels_path):
            print("Downloading imagelabels.mat...")
            urllib.request.urlretrieve("https://www.robots.ox.ac.uk/~vgg/data/flowers/102/imagelabels.mat", labels_path)
        if not os.path.exists(splits_path):
            print("Downloading setid.mat...")
            urllib.request.urlretrieve("https://www.robots.ox.ac.uk/~vgg/data/flowers/102/setid.mat", splits_path)
            
        self.labels_mat = loadmat(labels_path)['labels'][0] - 1
        splits_mat = loadmat(splits_path)
        
        if is_train:
            self.indices = splits_mat['trnid'][0].tolist()
        else:
            self.indices = splits_mat['valid'][0].tolist() 
    ...
    def __getitem__(self, idx):
        ...
        label = int(self.labels_mat[img_id - 1])
...
train_ds = OxfordFlowersDataset(DATA_DIR, is_train=True, transform=transform_train)
val_ds = OxfordFlowersDataset(DATA_DIR, is_train=False, transform=transform_val)
...
# =============================================================================
# 5. METRICS EVALUATION FUNCTION
# =============================================================================
def evaluate_comprehensive_metrics(model, dataloader, device):
    """Calculates all requested advanced metrics."""
    model.eval()
    all_preds = []
    all_targets = []
    
    # 1. Top-k Accuracies & Collection
    correct_top1 = 0
    correct_top5 = 0
    total = 0
    
    # 2. Performance Tracking (Throughput & Latency)
    ...
    # Sklearn Metrics (Mean Class, Macro P/R/F1)
    ...
    latency = (total_time / total) * 1000 # milliseconds per image
...
def train_phase(epochs, optimizer, scheduler, stage, phase_name):
    ...
    for epoch in range(epochs):
        ...
        # Validation (Fast version for training loop)
        model.eval()
        correct, total = 0, 0
        with torch.no_grad():
        ...
        epoch_end_time = time.time() # Stop timing the epoch
...
if __name__ == "__main__":
    ...
    total_start_time = time.time() # Start timing total execution
    ...
    # Log to screen and main log file
    log_to_file("================ FINAL METRICS ================")
    ...
    # Save specifically to the new metrics file
    ...
    total_end_time = time.time() # End timing total execution
