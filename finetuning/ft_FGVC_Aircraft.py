...
# Pointing to the inner 'data' folder as seen in the screenshot
DATA_DIR = os.path.join(PARENT_DIR, "data/fgvc_aircraft/data")
...
# =============================================================================
# 2. DATA HANDLING (FGVC-AIRCRAFT FORMAT)
# =============================================================================
class FGVCAircraftDataset(Dataset):
    def __init__(self, root_dir, is_train=True, transform=None):
        ...
        # 1. Load classes from variants.txt and map them to integer labels (0 to 99)
        variants_file = os.path.join(root_dir, 'variants.txt')
        with open(variants_file, 'r') as f:
            classes = [line.strip() for line in f.readlines()]
        self.class_to_idx = {cls_name: i for i, cls_name in enumerate(classes)}
        
        # 2. Determine target split file
        split_file = 'images_variant_train.txt' if is_train else 'images_variant_val.txt'
        split_path = os.path.join(root_dir, split_file)
        
        # 3. Parse the image IDs and their corresponding variants
        self.data = []
        with open(split_path, 'r') as f:
            for line in f:
                # FGVC text files are formatted as: "IMAGE_ID CLASS_NAME"
                # We split by the first space only, as class names can contain spaces (e.g. "Boeing 737")
                parts = line.strip().split(' ', 1)
                if len(parts) == 2:
                    img_id, variant_name = parts[0], parts[1]
                    img_name = f"{img_id}.jpg" # Append .jpg based on screenshot
                    img_path = os.path.join(self.images_dir, img_name)
                    
                    if variant_name in self.class_to_idx:
                        self.data.append({
                            'path': img_path,
                            'label': self.class_to_idx[variant_name]
                        })
...
train_ds = FGVCAircraftDataset(DATA_DIR, is_train=True, transform=transform_train)
val_ds = FGVCAircraftDataset(DATA_DIR, is_train=False, transform=transform_val)
...
if __name__ == "__main__":
    ...
    # Using a simple StepLR or Linear Warmup to rapidly stabilize the head
    ...
    # Run evaluation and save to evaluation_metrics_AGMBiN_FGVC_Aircraft.txt
